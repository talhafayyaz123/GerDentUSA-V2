...
const RecentOrders = (props: any) => {
    ...
    useEffect(() => {
        const getDashboard = async () => {
            ...
            let res: any = await fetch(`${SITE_URL}api/dashboard`, {
                method: 'POST',
            ...
            await setDashboardLoading(false)
        }
        getDashboard()
    ...
    return (
        <div className="table-heading-container">
            {
                orders.length==0 ? (
                    <div><strong>Sorry!</strong>, no recent orders found.</div>
                ) : (
                    <>
                    ...
                    </div>
                    {
                        orders.map((order: any, index: any) => {
                            return (<div key={index} className="db-detail-wrapper text-gray-600 text-sm">
